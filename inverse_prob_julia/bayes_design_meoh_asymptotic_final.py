...
RESULTS_FOLDER_NAME = "bayes_design_meoh_asymptotic_final"
...
def run_experiment_candidate_worker(args):
    x, noise_level = args
    y_next, Yexp_next = run_experiment(np.asarray(x, dtype=float), noise_level)
    return y_next, Yexp_next


def evaluate_experiment_batch(X_batch, noise_level):
    ...
    if (not CANDIDATES_IN_PARALLEL) or len(X_batch) <= 1:
        ...
        return y_new_list, Yexp_new_list
    ...
    with ProcessPoolExecutor(max_workers=max_workers, mp_context=ctx) as executor:
        for x_next, (y_next, Yexp_next) in zip(
            X_batch,
            executor.map(run_experiment_candidate_worker, args),
        ):
            print(f"  x={x_next}, methanol={y_next:.6f}")
            y_new_list.append(y_next)
            Yexp_new_list.append(Yexp_next)
...
def bayesian_optimization(
    ...
    # BO iterations with a configurable selected-candidate batch per iteration
    for iteration in range(max_experiments - N_init):
        ...
        y_new_list, Yexp_new_list = evaluate_experiment_batch(
            X_batch,
            noise_level,
        )
...
def plot_mixed_log_error(all_results, results_dir=None):
    ...
    plt.title("Overall Parameter Error")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    save_and_show(fig, results_dir, "08_overall_mixed_parameter_error.png")
