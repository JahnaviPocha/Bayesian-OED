...
function main(; nref=1, T=493, P_total=10, levels=1, RBS=false, St=0.0, ratio=0.01, Mtcat=1, initvalue=0.1, k0=0.0, Ea=0.0, RBS_full=false, catcell=1, inlet_MFs=[0.3, 0.7, 0.0], strategy=nothing, unknown_storage=:dense, assembly=:cellwise)
    ...
    @show D_mix_avg = mixture_average_diffusivity(D, inlet_MFs, molar_weights; nospec)
        ...
        for i in 1:(nospec)
            ...
            if RBS == true
                if RBS_full == true
                    ...
                    boundary_neumann!(y, u, node, species=i, region=5, value=1.0 * L_BFs(node[1], levels, λ, L))# stoichiometry to be added to RBS formulation later  (0 * meth_syn(u, T, Mtcat, density) * St[1, i] + St_Inv[i] + 0 * RWGS(u, T, Mtcat, density) * St[2, i]))#* MWs[i] / density)
                end
    ...
    data = nothing
    ysys = VoronoiFVM.System(grid; flux=fitting_flux!,
...
function experiments(; Y_in, Temp, Nexps, ratio, N_repeats, std_data, Nspec)
    ...
    for i in 1:Nexps
        ...
        mixture_density[i] = density_rechner(molar_weights, Y_in[:, i], P_total, Temp[i])
        d = main(nref=2500, inlet_MFs=Y_in[:, i], ratio=ratio, St=St, k0=k_meth_rxn, T=Temp[i]) #T + i * ΔT
        Yout = youts(d, Nspec=Nspec) #yout_weighted(d;Nspecs=Nspec) #
        Ycat = ycat(d, Nspec=Nspec)
        for j in 1:Nspec
            Yexp[j, i] = Yout[j]
            Ycat_exp[j, i] = Ycat[j]
            #  ∇Yout[j, i] = Yout_ROM[j] - Yout[j]
        end
    end
    σ_data = std_data
...
function parameter_estimator(; ratio, nspec, Y_in, Temp, St, nref=2500, nreac, Nexps, Y_out, unknown_parameters, IG, N_repeats, σ_data, RBS_full=false)
    single_snapshot_A = []
    single_snapshot_B = []
    rbs_snapshot = []
    srbs_time = 0.0
    ...
    for j in 1:Nexps
        ...
        mixture_density[j] = density_rechner(molar_weights, Y_in[:, j], P_total, Temp[j])
    ...
    if RBS_full == true
        ...
        k, _ = newton_optimizer(single_snapshot_A, single_snapshot_B, Y_in, Y_out; mixture_density=mixture_density, Initial_Guess=IG, molar_weights=molar_weights, B_RBS=B_RBS, st=St, cov=V, dof=unknown_parameters, RBS=true, print=true, lm=true, T=Temp, Nexps=Nexps, N_measurements=N_repeats)
    else
    ...
    lower = [1e-2, 1e0, 1e-4, 1e-12, 1e7, 1e1, 1e1, 1e2, -2e5]
    upper = [1e2, 1e4, 1e2, 1e-8, 1e11, 1e5, 1e5, 1e6, -2e1]
    par = k .* (upper .- lower) .+ lower
    return par
end


#RUN this function to exectute the complete parameter estimation workflow 
function complete_workflow(; Nexps=10, ratio=0.01, nparas=9, std_data=1e-6, RBS_full=false)
    St = [-1 -3 1 1 0 0; -1 -1 1 0 1 0]
    nspec = size(St, 2)
    nreac = size(St, 1)
    IG = fill(0.1, 9)
    Y_in, Temp = random_points_generator(Nexps=Nexps, nspecs=nspec, lb=[0.1, 0.2, 0.0, 0.0, 0.0, 300], ub=[0.4, 0.5, 0.01, 0.01, 0.01, 600], Sampling=HaltonSample())
    Y_out = experiments(; Y_in=Y_in, Temp=Temp, Nexps=Nexps, ratio=ratio, N_repeats=50, std_data=std_data, Nspec=nspec)
    k = parameter_estimator(; ratio=ratio, nspec=nspec, Y_in=Y_in, Temp=Temp, St=St, nref=2500, nreac=nreac, Nexps=Nexps, Y_out=Y_out, unknown_parameters=nparas, IG=IG, N_repeats=50, σ_data=std_data, RBS_full=RBS_full)
    return k
end
