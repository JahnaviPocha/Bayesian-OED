# -*- coding: utf-8 -*-
"""
Cleaned Fisher-information Bayesian OED driver for the 3-species ROM/RBS model.

This is ROM mass-kinetics code, not methanol kinetics:
- 3 species
- 2 unknown kinetic parameters, k1 and k2
- design vector x = [Y1, Y2, Temp]
- Y3 is completed as 1 - Y1 - Y2

The FIO objective is local D-optimal information:
    F(x) = N_repeats / sigma^2 * J(x).T @ J(x)
    info(x) = 0.5 * logdet(Prior + F(x)) - 0.5 * logdet(Prior)

The GP/EI surrogate is trained on pointwise information values. Parameter
