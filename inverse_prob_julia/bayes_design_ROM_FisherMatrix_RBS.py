# -*- coding: utf-8 -*-
"""
Created on Thu Apr 30 14:23:05 2026
...
def compute_sensitivity(x, params):
    """
    ...
    for i in range(len(params)):
        ...
        # ==========================
        # Adaptive epsilon (KEY FIX)
        # ==========================

        eps_i = 0.2 * abs(params[i]) + 1e-6
        ...
        # ==========================
        # Use eps_i here (not eps)
        # ==========================
        J[:, i] = ((y_pert_mean - base_mean) / eps_i) * params[i]
...
def select_next_experiment_oed(X_existing, params, n_candidates=50):
    """
...
def check_parameter_convergence(param_history, tol=1e-3, patience=3):

    if len(param_history) < patience + 1:
        return False

    # Take last few parameter updates
    recent = param_history[-(patience+1):]

    deltas = []

    for i in range(1, len(recent)):
        delta = np.linalg.norm(recent[i] - recent[i-1])
        delta_rel = delta / (np.linalg.norm(recent[i-1]) + 1e-12)
        deltas.append(delta_rel)

    avg_delta = np.mean(deltas)

    print(f"Avg relative change (last {patience}): {avg_delta:.4e}")

    return avg_delta < tol
