...
RESULTS_FOLDER_NAME = "bayes_design_meoh_FIO_asymptotic_final"
...
def run_fio_candidate_worker(args):
    x, noise_level = args
    x = np.asarray(x, dtype=float)
    ...
    info_val, F_val = point_information_objective(
        x,
        noise_level=noise_level,
        theta_ref=NOMINAL_THETA_FOR_INFORMATION,
    )

    return methanol_val, Yexp, info_val, F_val


def evaluate_fio_candidate_batch(X_batch, noise_level):
    ...
    if (not CANDIDATES_IN_PARALLEL) or len(X_batch) <= 1:
        for x_next in X_batch:
            methanol_val, Yexp, info_val, F_val = run_fio_candidate_worker(
                (x_next, noise_level)
            )
            y_new_list.append(methanol_val)
            Yexp_new_list.append(Yexp)
            info_new_list.append(info_val)
            F_new_list.append(F_val)
            print(
                f"  Candidate done: methanol={methanol_val:.6f}, "
                f"info={info_val:.6f}"
            )
        return y_new_list, Yexp_new_list, info_new_list, F_new_list
    ...
    with ProcessPoolExecutor(max_workers=max_workers, mp_context=ctx) as executor:
        for methanol_val, Yexp, info_val, F_val in executor.map(
            run_fio_candidate_worker,
            args,
        ):
            y_new_list.append(methanol_val)
            Yexp_new_list.append(Yexp)
...
def BO(
    ...
    while len(X) < max_experiments:
        ...
        top_k = min(BO_BATCH_SIZE, remaining, len(sorted_idx))
        ...
        (
            ...
            info_new_list,
            F_new_list,
        ) = evaluate_fio_candidate_batch(X_batch, noise_level)
...
def write_summary_text(results):
    lines = [
        ...
        f"BO_BATCH_SIZE: {BO_BATCH_SIZE}",
        f"CANDIDATES_IN_PARALLEL: {CANDIDATES_IN_PARALLEL}",
        f"MAX_CANDIDATE_WORKERS: {MAX_CANDIDATE_WORKERS}",
...
def plot_relative_parameter_errors(results):
    for param_index in range(N_UNKNOWN_PARAMETERS):
        ...
        for noise, data in results.items():
            params = np.asarray(data["param_history"], dtype=float)
            exp_counts = np.asarray(data["param_exp_counts"], dtype=int)
            true_val = TRUE_PARAMS_PHYSICAL[param_index]

            estimate_display, ylabel_base, _ = transformed_parameter_values(
                params[:, param_index],
                param_index,
            )
            true_display, _, _ = transformed_parameter_values(
                [true_val],
                param_index,
            )
            
            rel_err = estimate_display - true_display[0]
            
            if param_index < 5:
                error_ylabel = "ln(estimate) - ln(true)"
            else:
                error_ylabel = "estimate - true"
            style = style_for_noise(noise)
...
def plot_mixed_log_parameter_error(results):
    ...
    plt.yscale("log")
    plt.xlabel("Total experiments used")
    plt.ylabel("RMS mixed parameter error")
    plt.title("Methanol FIO Overall Parameter Error")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    save_and_show(fig, RESULTS_DIR, "03_overall_mixed_parameter_error.png")
...
def plot_methanol_outputs(results):
    ...
    plt.title("CH3OH Outlet Fraction at Selected FIO Designs")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
